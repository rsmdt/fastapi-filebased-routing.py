"""Tests for class-based route definitions (RouteConfig + Route)."""
...
# === normalize_middleware tests (shared definition-time vocabulary) ===
...
class TestNormalizeMiddleware:
    ...
    def test_none_returns_empty_tuple(self) -> None:
        ...
        from fastapi_filebased_routing.core.routes import normalize_middleware
...
class TestValidateMiddlewareEntries:
    """Tests for the validate_middleware_entries utility function."""

    def test_async_entries_pass(self) -> None:
        """All-async middleware entries validate without error."""
        from fastapi_filebased_routing.core.routes import validate_middleware_entries

        async def mw(request: Any, call_next: Any) -> Any:
            return await call_next(request)

        # Should not raise
        validate_middleware_entries((mw,), source="test")

    def test_sync_entry_raises(self) -> None:
        """A sync middleware entry raises RouteValidationError."""
        from fastapi_filebased_routing.core.routes import validate_middleware_entries
        from fastapi_filebased_routing.exceptions import RouteValidationError

        def sync_mw(request: Any, call_next: Any) -> Any:
            return call_next(request)

        with pytest.raises(RouteValidationError, match="must be async"):
            validate_middleware_entries((sync_mw,), source="test")

    def test_non_callable_entry_raises(self) -> None:
        """A non-callable middleware entry raises RouteValidationError."""
        from fastapi_filebased_routing.core.routes import validate_middleware_entries
        from fastapi_filebased_routing.exceptions import RouteValidationError

        with pytest.raises(RouteValidationError, match="Non-callable middleware"):
            validate_middleware_entries(("not_a_function",), source="test")

    def test_custom_error_class_used(self) -> None:
        """The error_class argument controls the raised exception type."""
        from fastapi_filebased_routing.core.routes import validate_middleware_entries
        ...
        with pytest.raises(MiddlewareValidationError, match="must be async"):
            validate_middleware_entries(
                (sync_mw,), source="test", error_class=MiddlewareValidationError
            )
